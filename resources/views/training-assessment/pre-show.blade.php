    ...
    <div class="row justify-content-center">
        <div class="col-12 col-md-10 col-lg-8 col-xl-6">
            ...
            <form id="assessmentForm">
                @php
                    $groupedQuestions = $trainingAssessment->pre_questions->groupBy('question_category');
                    $questionNo = 1;
                @endphp
                ...
                @foreach ($groupedQuestions as $category => $questions)
                    ...
                    @endforeach
                @endforeach
                @if ($trainingAssessment->pre_submitted_on)
                    <div class="row">
                        <div class="col-12 text-end">
                            <p style="font-style: italic">Form Submitted At: {{$trainingAssessment->pre_submitted_on->format('d/m/Y H:i:s')}}</p>
                        </div>
                    </div>
                @endif
                @if ($trainingAssessment->status == 1 && $trainingAssessment->user_id == request()->user()->id)
                    <div class="row">
                        <div class="col-12 text-end">
    ...
    @push('scripts')
        <script>
            $(document).on('click', '.submit-btn', function (e) {
                ...
                $.ajax({
                    url: "/training-assessment/" + {{$trainingAssessment->id}} + "/pre/submit",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
