    ...
    <div class="row justify-content-center">
        <div class="col-12 col-md-10 col-lg-8 col-xl-6">
                ...
                <div class="card-body p-0">
                    <div class="p-3" style="border-bottom: 1px solid #dee2e6">
                        <div class="row align-items-center">
                            <div class="col-12">
                                <h5 class="mb-3">TRAINING EFFECTIVENESS EVALUATION FORM</h5>
                                <p class="mb-1">Course Title: {{$trainingEvaluation->form_title}}</p>
                                <p class="mb-1">Course Venue: {{$trainingEvaluation->form_venue ?? ''}}</p>
                                <p class="mb-0">Course Provider: {{$trainingEvaluation->form_provider ?? ''}}</p>
                            </div>
                        </div>                
                    </div>
                </div>
            ...
            <form id="evaluationForm">
                @php
                    $groupedQuestions = $trainingEvaluation->questions->groupBy('question_category');
                    $questionNo = 1;
                @endphp
                ...
                @foreach ($groupedQuestions as $category => $questions)
                                                ...
                                                @for ($i = 1; $i <= 5; $i++)
                                                    <div class="form-check form-check-inline">
                                                        <input
                                                            ...
                                                            @if($trainingEvaluation->status != 1 || $trainingEvaluation->user_id != request()->user()->id) disabled @endif
                                                        >
                    ...
                    @endforeach
                @endforeach
                @if ($trainingEvaluation->submitted_on)
                    <div class="row">
                        <div class="col-12 text-end">
                            <p style="font-style: italic">Form Submitted At: {{$trainingEvaluation->submitted_on->format('d/m/Y H:i:s')}}</p>
                        </div>
                    </div>
                @endif
                @if ($trainingEvaluation->status == 1 && $trainingEvaluation->user_id == request()->user()->id)
                    <div class="row">
                        <div class="col-12 text-end">
    ...
    @push('scripts')
        <script>
            $(document).on('click', '.submit-btn', function (e) {
                ...
                $.ajax({
                    url: "/training-evaluation/" + {{$trainingEvaluation->id}} + "/submit",
                    type: "POST",
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function (response) {
                        toastr.success('Training Evaluation submitted successfully!', '', { timeOut: 8000 });
